...
void kernel_double(double *Y, const double *X, mwSize N,
    ...
    for (mwSize i = 0; i < N; i++) {
        ...
        if (x < -1.0) {
            double Xp = -0.5 * x - 0.5;
            double G  = 1.0 / (1.0 + km1 * exp(alpha * Xp));
            Y[i] = k * G * (1.0 - alpha * Xp * (1.0 - G)) * slope;
        }
        else if (x > 1.0) {
            double Xq = 0.5 * x - 0.5;
            double G  = 1.0 / (1.0 + km1 * exp(alpha * Xq));
            Y[i] = k * G * (1.0 - alpha * Xq * (1.0 - G)) * slope;
        }
        else {
            Y[i] = slope;
        }
...
void kernel_float(float *Y, const float *X, mwSize N,
    ...
    for (mwSize i = 0; i < N; i++) {
        float x = slope * X[i];
        if (x < -1.0f) {
            float Xp = -0.5f * x - 0.5f;
            float G  = 1.0f / (1.0f + km1 * expf(alpha * Xp));
            Y[i] = k * G * (1.0f - alpha * Xp * (1.0f - G)) * slope;
        }
        else if (x > 1.0f) {
            float Xq = 0.5f * x - 0.5f;
            float G  = 1.0f / (1.0f + km1 * expf(alpha * Xq));
            Y[i] = k * G * (1.0f - alpha * Xq * (1.0f - G)) * slope;
        }
        else {
            Y[i] = slope;
        }
