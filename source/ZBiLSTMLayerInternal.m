classdef ZBiLSTMLayerInternal < nnet.internal.cnn.layer.FunctionalStatefulLayer ...
    ...
    methods
        function this = ZBiLSTMLayerInternal(name, inputSize, hiddenSize, ...
                rememberCellState, rememberHiddenState, returnSequence, ...
                activations, actParameters, customLearnableParametersSize, generalStrategy, hasStateInputs, hasStateOutputs)
            % ZBiLSTM   Constructor for an BiLSTM layer
            %
            ...
            this.CustomLearnableParameters.Initializer = iInternalInitializer('narrow-normal');  
        ...
        function Zs = forwardExampleInputs(this, Xs)
            ...
            % The output of BiLSTM has no spatial dimensions, the number of
            % output channels is twice the hidden size, and the sequence
            % length either stays unchanged (OutputMode='sequence') or is
            % reduced down to one (OutputMode='last').
            Z = setSizeForDim(X,'S',[]);
            Z = setSizeForDim(Z,'C',2*this.HiddenSize);
        ...
        function out = forwardPropagateSequenceLength(~, ~, ~)
            ...
            error("Temporary internal error: forwardPropagateSequenceLength "+...
                "should not be called on a BiLSTM layer anymore")
        end
        ...
        function this = initializeLearnableParameters(this, precision)
            ...
            if isempty(this.InputWeights.Value)
                ...
                weightsSize = [8*this.HiddenSize, this.InputSize];  % 8 = 4 x 2 veces
            ...
            if isempty(this.RecurrentWeights.Value)
                % Initialize only if it is empty
                weightsSize = [8*this.HiddenSize, this.HiddenSize];  % 8 = 4 x 2 veces
            ...
            if isempty(this.Bias.Value)
                % Initialize only if it is empty
                biasSize = [8*this.HiddenSize, 1];  % 8 = 4 x 2 veces
...
%                     customLearnableParameters = rand(this.CustomLearnableParametersSize) .* 0.05 + 0.05;
                    customLearnableParameters = zeros(this.CustomLearnableParametersSize) .* 0;
                else
        ...
        function state = computeState(this, ~, Z, memory, ~)
            ...
            if ~this.HasStateInputs
                ...
                if this.HasStateOutputs
                    ...
                    cellState = memory.CellState;
                    hiddenState = memory.HiddenState;
                end
                CfS = cellState(1:this.HiddenSize, :, end);
                HfS = hiddenState(1:this.HiddenSize, :, end);
        ...
        function this = set.Learnables(this, learnables)
            ...
            nnet.internal.cnn.layer.paramvalidation.assertValidLearnables(learnables{1}, [8*hiddenSize, this.InputSize]);
            nnet.internal.cnn.layer.paramvalidation.assertValidLearnables(learnables{2}, [8*hiddenSize, hiddenSize]);
            nnet.internal.cnn.layer.paramvalidation.assertValidLearnables(learnables{3}, [8*hiddenSize, 1]);
            if ~isempty(this.CustomLearnableParametersSize)
        ...
        function this = set.State(this, state)
            ...
            if this.HasStateInputs && ~all(marker)
                error(message('nnet_cnn:internal:cnn:layer:BiLSTM:SettingStateWithStateInputs'));
            end
            hiddenState = state{1};
            cellState = state{2};
            ...
            expectedAttributes = {'size', [2*this.HiddenSize NaN]};
            if ~marker(1)
    ...
    methods (Access = private)
        function strategy = getHostStrategy(this)
            ...
            elseif this.ReturnSequence
                strategy = nnet.internal.cnn.layer.util.BiLSTMHostStrategy();
            else
                strategy = nnet.internal.cnn.layer.util.BiLSTMHostReturnLastStrategy();
            end
        end
        ...
        function strategy = getGPUStrategy(this)
            if needsGeneralStrategy(this)
                strategy = this.getGeneralStrategy();
            elseif this.ReturnSequence
                strategy = nnet.internal.cnn.layer.util.BiLSTMGPUStrategy();
            else
                strategy = nnet.internal.cnn.layer.util.BiLSTMGPUReturnLastStrategy();
            end
        end
        ...
        function assertIsConsistentWithInferredSize(this,featureSize)
            assert( ~this.HasSizeDetermined || ...
                isequal(this.InputSize, prod(featureSize)) )
        end
        ...
        function assertValidStateInput(this, Xs)
            ...
            if ~isequal(stateSize, 2*this.HiddenSize)
    ...
    methods (Access = protected)
        function this = setFunctionalStrategy(this)
            ...
            %    error(message('nnet_cnn:internal:cnn:layer:BiLSTM:NonDefaultActivFunctional'))
            %end
            
            if this.ReturnSequence
                strategy = nnet.internal.cnn.layer.util.BiLSTMFunctionalStrategy;
            else
                strategy = nnet.internal.cnn.layer.util.BiLSTMFunctionalReturnLastStrategy;
            end
...
error( message('nnet_cnn:internal:cnn:layer:BiLSTM:NonScalarInputSize', ...
    iSizeToString(inputSize)) );
end

function iThrowStateInputMustBeCB()
error( message('nnet_cnn:internal:cnn:layer:BiLSTM:StateInputMustBeCB') );
end

function iThrowStateInputMustMatchHiddenSize()
error( message('nnet_cnn:internal:cnn:layer:BiLSTM:StateInputMustMatchHiddenSize') );
end
