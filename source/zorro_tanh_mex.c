...
void kernel_double(double *Y, const double *X, mwSize N,
    ...
    for (mwSize i = 0; i < N; i++) {
        ...
        if (x < -1.0) {
            double Xp = x + 1.0;
            Y[i] = k * Xp / (1.0 + km1 * exp(-0.5 * alpha * Xp)) - 1.0;
        }
        else if (x > 1.0) {
            double Xq = 1.0 - x;
            Y[i] = 1.0 - k * Xq / (1.0 + km1 * exp(-0.5 * alpha * Xq));
        } 
        else {
            Y[i] = x;
        }
...
void kernel_float(float *Y, const float *X, mwSize N,
    ...
    for (mwSize i = 0; i < N; i++) {
        ...
        if (x < -1.0f) {
            float Xp = x + 1.0f;
            Y[i] = k * Xp / (1.0f + km1 * expf(-0.5f * alpha * Xp)) - 1.0f;
        }
        else if (x > 1.0f) {
            float Xq = 1.0f - x;
            Y[i] = 1.0f - k * Xq / (1.0f + km1 * expf(-0.5f * alpha * Xq));
        } 
        else {
            Y[i] = x;
        }
